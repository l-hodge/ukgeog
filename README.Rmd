...
<!-- badges: start -->
  [![Travis build status](https://travis-ci.org/l-hodge/ukgeog.svg?branch=master)](https://travis-ci.org/l-hodge/ukgeog)
...
The `read_sf` function can be used to download boundaries for the following geographies using the 'geog' argument:
...
available_sf() %>%
    ...
    dplyr::summarize(year = stringr::str_c(year, collapse = "; ")) %>% 
